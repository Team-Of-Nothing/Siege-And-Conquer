...
public class BattleScreen implements Screen {
    ...
    BattleScreen(final SAC game) {
        ...
        Array<Integer> army2IDs = new Array <Integer>(game.enemy.getArmy().size());
        for (int i = 0; i < game.enemy.getArmy().size(); i++) {
            army2IDs.add(game.enemy.getArmy().get(i).getId());
        }
    ...
    public void render(float delta) {
        ...
        stage.draw();
        if (time > 1.5f) {
            if (k < game.player.getArmy().size() && l < game.enemy.getArmy().size()) {
